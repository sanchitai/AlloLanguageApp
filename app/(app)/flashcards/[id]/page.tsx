...
import { getFlashCards, getStoredLang, getLangLabel } from '@/lib/content'

type CardState = 'unseen' | 'learned' | 'review'

export default function FlashcardsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter()
  const [scenarioId, setScenarioId] = useState('preset-daycare')
  const lang = getStoredLang()
  const langLabel = getLangLabel(lang)
  const CARDS = getFlashCards(scenarioId, lang)

  const [idx, setIdx] = useState(0)
  const [flipped, setFlipped] = useState(false)
  const [states, setStates] = useState<Record<string, CardState>>(() =>
    Object.fromEntries(CARDS.map(c => [c.id, 'unseen' as CardState]))
  )
  ...
  function advance(dir: 'left' | 'right') {
    const newStates: Record<string, CardState> = {
      ...states,
      [card.id]: (dir === 'right' ? 'learned' : 'review') as CardState,
    }
    setStates(newStates)
  ...
  if (done) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100%', padding: 32, gap: 24, textAlign: 'center', background: 'var(--bg)' }}>
      <div style={{ fontSize: 80, animation: 'bounce-in 0.7s var(--spring) both' }}>🏆</div>
      <div>
        <div style={{ fontSize: 34, fontWeight: 900, letterSpacing: '-0.028em', color: 'var(--ink)' }}>Session Complete!</div>
        <div style={{ fontSize: 15, color: 'var(--ink-2)', marginTop: 8, fontWeight: 600 }}>You went through all {CARDS.length} cards.</div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, width: '100%' }}>
        <div style={{ background: 'var(--tile-green)', borderRadius: 'var(--r-xl)', padding: '16px 10px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--tile-green-ink)' }}>Learned</div>
        </div>
        <div style={{ background: 'var(--tile-yellow)', borderRadius: 'var(--r-xl)', padding: '16px 10px', textAlign: 'center' }}>
          <div style={{ fontSize: 28, fontWeight: 900, color: 'var(--tile-yellow-ink)' }}>{CARDS.length - learned}</div>
          <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--tile-yellow-ink)' }}>Review</div>
        </div>
        <div style={{ background: 'var(--primary)', borderRadius: 'var(--r-xl)', padding: '16px 10px', textAlign: 'center' }}>
          <div style={{ fontSize: 28, fontWeight: 900, color: '#fff' }}>+{learned * 5}</div>
          <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'rgba(255,255,255,0.75)' }}>XP</div>
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%' }}>
        <button
          onClick={() => { setIdx(0); setFlipped(false); setDone(false); setStates(Object.fromEntries(CARDS.map(c => [c.id, 'unseen' as CardState]))) }}
          style={{ height: 54, background: 'var(--surface)', border: '2px solid var(--divider)', borderRadius: 9999, fontSize: 16, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font)', color: 'var(--ink)' }}
        >Practice Again</button>
        <button onClick={() => router.back()} style={{ height: 54, background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: 9999, fontSize: 16, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font)', boxShadow: 'var(--sh-fab)' }}>
          Back to Scenario
        </button>
      </div>
    </div>
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: 'var(--surface)', display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px 14px', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, textAlign: 'center' }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--ink)' }}>Flashcards</div>
          <div style={{ fontSize: 12, color: 'var(--ink-3)', fontWeight: 600 }}>{langLabel.flag} {langLabel.name}</div>
        </div>
        <button onClick={() => router.back()} style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--surface-alt)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ background: 'var(--surface)', padding: '0 20px 16px', flexShrink: 0, borderBottom: '1px solid var(--divider)' }}>
        <div style={{ height: 7, background: 'var(--primary-light)', borderRadius: 9999, overflow: 'hidden', marginBottom: 8 }}>
          <div style={{ height: '100%', background: 'linear-gradient(90deg,var(--sky),var(--primary))', borderRadius: 9999, width: `${pct}%`, transition: 'width 400ms var(--ease-out)' }} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, fontWeight: 700, color: 'var(--ink-3)' }}>
          ...
          <span style={{ color: 'var(--green)', fontWeight: 800 }}>{learned} learned</span>
        </div>
      </div>

      {/* Card arena */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px 20px 8px', position: 'relative' }}>
        ...
        {idx + 1 < CARDS.length && <div style={{ position: 'absolute', width: 'calc(100% - 60px)', maxWidth: 360, height: 430, borderRadius: 32, background: 'linear-gradient(145deg,#f0f0f8,#e8e8f4)', transform: 'translateY(10px) scale(0.96)', opacity: 0.7, zIndex: 1 }} />}
        {idx + 2 < CARDS.length && <div style={{ position: 'absolute', width: 'calc(100% - 60px)', maxWidth: 360, height: 430, borderRadius: 32, background: 'linear-gradient(145deg,#e8e8f4,#e0e0ee)', transform: 'translateY(20px) scale(0.92)', opacity: 0.4, zIndex: 0 }} />}
            ...
            <div onClick={() => { if (!isDragging && Math.abs(dragX) < 5) setFlipped(true) }} style={{ position: 'absolute', inset: 0, borderRadius: 32, backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', background: card.grad, boxShadow: '0 20px 60px rgba(0,0,0,0.18)', overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: 22 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', position: 'relative', zIndex: 1 }}>
                <div style={{ background: 'rgba(255,255,255,0.28)', border: '1px solid rgba(255,255,255,0.35)', borderRadius: 9999, padding: '5px 12px', fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.9)' }}>
                  {langLabel.flag} {langLabel.native}
                </div>
                <div style={{ background: 'rgba(0,0,0,0.12)', borderRadius: 9999, padding: '4px 10px', fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.55)' }}>{idx + 1}/{CARDS.length}</div>
              </div>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, position: 'relative', zIndex: 1 }}>
                <div style={{ fontSize: 42, fontWeight: 900, letterSpacing: '-0.025em', color: '#fff', textAlign: 'center', textShadow: '0 2px 16px rgba(0,0,0,0.15)' }}>{card.target}</div>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.70)', background: 'rgba(0,0,0,0.15)', padding: '5px 13px', borderRadius: 9999 }}>
                  <svg viewBox="0 0 16 16" fill="none" width="13" height="13"><path d="M2 8a6 6 0 1 1 11.5 2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M10 10l3.5 0 0-3.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  Tap to reveal
                </div>
              </div>
            ...
            <div style={{ position: 'absolute', inset: 0, borderRadius: 32, backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)', background: 'var(--surface)', boxShadow: '0 20px 60px rgba(0,0,0,0.18)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <div style={{ background: card.band, padding: '18px 22px 16px', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.55)' }}>{idx + 1}/{CARDS.length}</div>
                </div>
      ...
      <div style={{ background: 'var(--surface)', borderTop: '1px solid var(--divider)', padding: '12px 20px 28px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 14 }}>
          {CARDS.map((c, i) => (
            <div key={c.id} style={{ borderRadius: 9999, width: i === idx ? 24 : 7, height: 7, background: i === idx ? 'var(--primary)' : states[c.id] === 'learned' ? 'var(--green)' : states[c.id] === 'review' ? 'var(--maple)' : 'var(--divider)', transition: 'all 300ms var(--spring)' }} />
          ))}
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => speak(card.target)} style={{ flex: 1, height: 48, background: 'var(--primary-light)', border: 'none', borderRadius: 'var(--r-lg)', fontSize: 12, fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontFamily: 'var(--font)', color: 'var(--primary-dark)' }}>
            <svg viewBox="0 0 20 20" fill="none" width="18" height="18"><polygon points="5,4 16,10 5,16" fill="currentColor"/></svg>
            Listen
          </button>
          <button style={{ flex: 1, height: 48, background: 'var(--surface-alt)', border: 'none', borderRadius: 'var(--r-lg)', fontSize: 12, fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontFamily: 'var(--font)', color: 'var(--ink-2)' }}>
